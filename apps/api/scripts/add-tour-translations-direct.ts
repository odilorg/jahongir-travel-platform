...
Независимо от того, являетесь ли вы опытным путешественником или новичком, ищущим приключений, этот тур предлагает идеальный баланс между вызовом и наградой. Каждый вечер возвращайтесь в уютные горные гостевые дома, где вы сможете расслабиться и насладиться традиционным узбекским гостеприимством.`,
    highlights: [
      'Восхождение на Большой Чимган (3309м)',
      'Прогулка на лодке по Чарвакскому водохранилищу',
      'Альпийские луга и полевые цветы',
      'Посещение традиционной горной деревни',
      'Ужин у костра под звёздами',
      'Профессиональный горный гид',
    ],
    included: [
      'Проживание в горном гостевом доме (2 ночи)',
    ...
    excluded: [
      'Личное треккинговое снаряжение (ботинки, палки)',
...
Tajribali sayohatchi bo'lsangiz yoki sarguzasht izlayotgan yangi boshlovchi bo'lsangiz ham, bu sayohat qiyinchilik va mukofotning mukammal muvozanatini taklif etadi. Har oqshom qulay tog' mehmonxonalariga qaytib, dam olishingiz va an'anaviy o'zbek mehmondo'stligidan bahramand bo'lishingiz mumkin.`,
    highlights: [
      "Katta Chimgon cho'qqisiga chiqish (3309m)",
      "Charvok suv ombori bo'ylab qayiqda sayohat",
      "Alp yaylovlari va yovvoyi gullar",
      "An'anaviy tog' qishlog'iga tashrif",
      "Yulduzlar ostida gulxan kechki ovqati",
      "Professional tog' gidi",
    ],
    included: [
      "Tog' mehmonxonasida turar joy (2 kecha)",
    ...
    excluded: [
      "Shaxsiy trekking jihozlari (etiklar, tayoqlar)",
      "Sayohat sug'urtasi",
      "Shaxsiy xarajatlar",
      "Gid uchun tip",
    ],
...
async function main() {
  console.log('Adding translations for tour:', TOUR_ID);

  for (const translation of translations) {
    const { locale, ...data } = translation;

    const result = await prisma.tourTranslation.upsert({
      where: {
        tourId_locale: {
          tourId: TOUR_ID,
          locale,
  ...
  allTranslations.forEach((t) => {
    console.log(`  - ${t.locale}: ${t.title} (${t.slug})`);
  });
}
